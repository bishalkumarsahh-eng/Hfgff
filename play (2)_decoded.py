...
from Elevenyts.helpers import buttons, utils, next_play_emoji
...
async def _play_animation(sent_msg, frame: str):
    """Re-affirm the searching message. Silently stops when message is edited externally."""
...
async def play_hndlr(
    ...
    try:
        sent = await safe_reply(m, _play_text)
    except FloodWait as e:
        await asyncio.sleep(e.value)
        try:
            sent = await safe_reply(m, _play_text)
    ...
    # Keep the message steady (stops itself when message is externally edited)
    if sent:
        asyncio.create_task(_play_animation(sent, _play_text))
